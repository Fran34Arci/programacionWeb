...
*/
var arreglo1 = new Array(4, 7, 9, 3, 1, 45, 67, 23, 29, 78, 11, 16);
...
function arregloDescendente(arrayAux) {
    for (var x = 0; x <= (arrayAux.length); x++) {
        for (var i = 0; i < arreglo1.length - x - 1; i++) {
            if (arrayAux[i] <= arrayAux[(i + 1)]) {
                var AuxCambio = arrayAux[i + 1];
                arrayAux[i + 1] = arrayAux[i];
                arrayAux[i] = AuxCambio;
            }
        }
    }
    var numMas = arrayAux[0];
